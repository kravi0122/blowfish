...
public class BlowfishTestVectors {
    private static boolean ecbTestsPass() {
        try {
            for(int i = 0; i < ecb_tests.length; ++i) {
                Blowfish bf = new Blowfish(BlockCipher64.longToBytes(ecb_tests[i][0]));
                long out = bf.encrypt(ecb_tests[i][1]);
                if(out != ecb_tests[i][2]) {
                    System.err.printf("TestVectors ecb_tests FAILED at test %d\n", i);
                    return false;
                }
            }
            return true;
    ...
    public static void main(String[] args) {
        if(!ecbTestsPass()) {
            System.err.println("Test vectors failed!");
            System.exit(-1);
        }
    }

    // {key, cleartext, ciphertext}
    private static final long ecb_tests[][] =
